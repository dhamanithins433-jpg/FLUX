...
export default function CSECourseHub({ onBackToCourses }) {
  ...
  // Initial Load: Fetch CSE Department & Regulations
  useEffect(() => {
    fetchCSEOverview();
  }, []);

  const fetchCSEOverview = async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/courses/cse`);
      if (!res.ok) throw new Error("Unable to load CSE Department details");
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsSearching(true);
      try {
        const res = await fetch(`${API_BASE}/courses/cse/search?q=${encodeURIComponent(searchQuery.trim())}`);
        if (res.ok) {
          const data = await res.json();
    ...
    <div className="cse-container">
      ...
      <section className="cse-hero-banner">
        <div className="cse-hero-content">
          ...
          <p>
            Welcome to the dedicated academic hub for CSE students. Explore comprehensive curricula, 
            syllabus blueprints, unit-wise lecture notes, university question banks, and verified learning resources.
          </p>
          ...
          <div className="cse-stats-chips">
            ...
            <div className="stat-chip">
              🎓 Semesters: <strong>1 to 8</strong>
            </div>
            <div className="stat-chip">
      ...
      <section className="cse-search-section">
        <div className="cse-search-box">
          ...
          <input
            type="text"
            placeholder="Search CSE subjects (e.g. CS3401, DBMS, Theory of Computation, Algorithms)..."
            value={searchQuery}
          ...
          <div className="cse-search-results-panel">
            ...
            {searchResults.total_matches === 0 ? (
              <div className="empty-materials-state">
                ...
                <p>Try searching with course code (e.g., CS3452) or general subject title (e.g., Algorithms, DBMS, Networks).</p>
              </div>
            ) : (
                  ...
                  >
                    <div>
                      ...
                      <ul className="reg-highlights-list">
                        ...
                        <li>
                          <span>✓</span> 8 Full Semesters with Curated Syllabi
                        </li>
                        <li>
                ...
                >
                  <div>
                    ...
                    <p className="sub-dept-label">
                      {sub.department || "Computer Science Engineering"} • Sem {sub.semester}
                    </p>
